...
import pandas as pd  # type: ignore  # noqa: E402, F811, I001
import matplotlib.pyplot as plt  # type: ignore # noqa: E402, F811
...
# 1. Count of customers by region and year
fig, axes = plt.subplots(2, 2, figsize=(15, 10))

# Extract year from JoinDate
df['JoinYear'] = df['JoinDate'].dt.year

# Subplot 1: Bar chart - Customers per region
ax1 = axes[0, 0]
df['Region'].value_counts().plot(kind='bar', ax=ax1, color='skyblue')
ax1.set_title('Number of Customers by Region')
ax1.set_xlabel('Region')
ax1.set_ylabel('Count')
ax1.tick_params(axis='x', rotation=45)

# Subplot 2: Heatmap - Region vs Year
ax2 = axes[0, 1]
region_year = pd.crosstab(df['Region'], df['JoinYear'])
sns.heatmap(
    region_year,
    annot=True,
    fmt='d',
...
ax2.set_title('Customers by Region and Join Year')

# Subplot 3: Line chart - Customer additions over time by region
ax3 = axes[1, 0]
for region in df['Region'].unique():
    region_data = df[df['Region'] == region].sort_values('JoinDate')
    region_data['CumulativeCount'] = range(1, len(region_data) + 1)
    ax3.plot(
        region_data['JoinDate'],
        region_data['CumulativeCount'],
        label=region,
        marker='o',
        linewidth=2,
    )
ax3.set_title('Cumulative Customer Growth by Region Over Time')
ax3.set_xlabel('Join Date')
ax3.set_ylabel('Cumulative Count')
ax3.legend()
ax3.grid(True, alpha=0.3)

# Subplot 4: Box plot - Join dates by region (as numeric for visualization)
ax4 = axes[1, 1]
df['DateNumeric'] = (df['JoinDate'] - df['JoinDate'].min()).dt.days
sns.boxplot(data=df, x='Region', y='DateNumeric', ax=ax4, palette='Set2')
ax4.set_title('Distribution of Join Dates by Region')
ax4.set_xlabel('Region')
ax4.set_ylabel('Days Since Earliest Join Date')
ax4.tick_params(axis='x', rotation=45)

plt.tight_layout()
plt.savefig('region_joindate_analysis.png', dpi=300, bbox_inches='tight')
plt.show()

print(f"\nSummary Statistics:")  # noqa: F541
print(
    df.groupby('Region')
    .agg({'CustomerID': 'count', 'JoinDate': ['min', 'max', 'mean']})
    .round()
)
