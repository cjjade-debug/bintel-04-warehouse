...
ax1.set_title(
    'Payment Type Distribution Over Join Dates (Stacked Area)',
    fontsize=14,
    fontweight='bold',
)
...
payment_counts = (
    df.groupby(['YearMonth', 'DefaultPaymentType']).size().unstack(fill_value=0)
)
most_common = payment_counts.idxmax(axis=1)

# Create line chart for Credit/Debit trend (most common)
credit_debit_data = (
    df[df['DefaultPaymentType'] == 'Credit/Debit'].groupby('YearMonth').size()
)
ax2_twin = ax2.twinx()
ax2_twin.plot(
    range(len(credit_debit_data)),
    credit_debit_data.values,
    'ro-',
    linewidth=2.5,
    markersize=6,
...
ax2.set_title(
    'Customer Join Dates vs Payment Type Trends (Combo: Bars + Line)',
    fontsize=14,
    fontweight='bold',
)
